 ...
 * @param {string} actionName The action type prefix (e.g., 'pokemon/fetchPokemon').
...
export const generateAsyncThunk = <TReturned, TArgument, TState>(
  ...
  const options = {
    ...
    condition: (
      ...
      console.log("condition state.asyncRequests ", state.asyncRequests);

      if (!state.asyncRequests) {
        return true;
      }

      const currLoadingState = state.asyncRequests[actionName];
  ...
  >(actionName, payloadCreator, options);
};
